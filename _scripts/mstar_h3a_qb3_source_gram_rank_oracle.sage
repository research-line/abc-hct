#!/usr/bin/env sage
# Compute/guard the Q_B-3 source-only Gram-rank target.
#
# This is the narrowed form after the rank(G) reduction:
#
#     A = C_source B_AL C_source^T,
#     target rank(A) = d - 1.
#
# Dense mode is a smoke/regression mode only.  Large levels must use a
# matrix-free or blockwise oracle.
...
def make_free_to_sage(pi_data, manifest):
...
def compute_source_rank(case_dir, pi_json, max_dense_dim):
    ...
    if free_dim > max_dense_dim:
        return {
            "tool": "mstar_h3a_qb3_source_gram_rank_oracle",
            ...
            "reason": "Use a matrix-free Source-only Gram-rank oracle for this dimension.",
        }
    ...
    bridge = make_free_to_sage(pi_data, manifest)
    F = bridge["field"]
    N = bridge["N"]
    q = bridge["q"]
    ...
    if sage_dim > max_dense_dim:
        return {
            ...
            "mode": "dense",
            "status": "blocked_by_dense_guard",
            "quotient_dim": sage_dim,
            "source_rows": source_rows,
            "rank_A_target": sage_dim - 1,
            "max_dense_dim": int(max_dense_dim),
            "reason": "Use a matrix-free Source-only Gram-rank oracle for this dimension.",
        }
    ...
    return {
        ...
        "status": "computed",
        "quotient_dim": sage_dim,
        "source_rows": source_dim,
        "source_linear_rank": rank_c,
        "rank_A": rank_a,
        "rank_A_target": source_dim,
        "rank_A_full": bool(rank_a == source_dim),
        "nullity_A": source_dim - rank_a,
...
def write_markdown(payload, out_md):
    lines = [
        "# H3a Q_B-3 Source-Gram Rank Oracle",
        "",
    ...
    if payload.get("status") == "computed":
        lines.extend([
            ...
            f"source row rank:    {payload['source_linear_rank']}",
            f"rank(A):            {payload['rank_A']} / {payload['rank_A_target']}",
            f"nullity(A):         {payload['nullity_A']}",
            f"rank(A) full:       {payload['rank_A_full']}",
            f"Bal rank:           {payload['Bal_rank']}",
            f"Bal det:            {payload['Bal_det_signed']}",
            "```",
            "",
            "Interpretation: dense smoke mode verifies the narrowed Q_B-3",
            "source-only Gram-rank target.",
            "",
        ])
    else:
        lines.extend([
            ...
            payload.get("reason", ""),
            "",
            f"Quotient dimension `{payload.get('quotient_dim')}` exceeds guard",
            f"`{payload.get('max_dense_dim')}`.",
            "",
            "The large-level target is only the Source-only block",
            f"`rank(A)={payload.get('rank_A_target')}`.",
            "",
        ])
...
def main():
    ...
    print(json.dumps({
        ...
        "rank_A_target": payload.get("rank_A_target"),
