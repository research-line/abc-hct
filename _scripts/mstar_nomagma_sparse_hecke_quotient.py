...
Alternative Frey tripel can be tested without editing this file:

    sage -python _scripts/mstar_nomagma_sparse_hecke_quotient.py \
      --backend sage --raw-a 1 --raw-b 4374 \
      --levels 3360 --modes raw anc --primes 5 7 11 13 \
      --q 3863 --out-json _results/mstar_nomagma_smoke_1_4374_N3360.json
...
def run_sage_level(
    ...
    def write_rc3_manifest() -> None:
        if rc3_run_dir is None:
            return
        manifest = {
            "certificate_version": "rc3a-row-transcript-draft-2",
            "level": N,
            "mode": mode,
            "sign": sign,
            "q": q,
            ...
            "scope": "row transcript hashes plus per-row hash indexes; not yet a pivot/rank witness",
        }
    ...
    def write_rc3_source_row(
        stage: str,
        stage_row_index: int,
        row: dict[int, Any],
        row_line_sha256: str | None = None,
        ...
        if row_line_sha256 is not None:
            record["row_line_sha256"] = row_line_sha256
        if row_metadata:
            record["row_metadata"] = row_metadata
    ...
    def write_rc3_source_manifest(stage: str) -> None:
        if rc3_source_run_dir is None or rc3_source_rows_path is None or py_ranker is None:
            return
        quotient_rank = quotient_ranker.rank if quotient_ranker is not None else 0
        full_rank = py_ranker.rank + quotient_rank
        if full_rank != ncols:
            return
        manifest = {
            ...
            "scope": (
                "original rows that increased the Python sparse base rank or "
                "the quotient-compressed rank; verifier recomputes full rank "
                "from original rows"
            ),
        }
    ...
    def unpack_row_item(item: Any) -> tuple[dict[int, Any], dict[str, Any] | None]:
        if (
            ...
            metadata = {str(k): v for k, v in item[0].items()}
            row = item[1]
            return row, metadata
        return item, None

    def add_rows_to_entries(stage: str, row_iter: Iterable[Any]) -> tuple[int, int]:
        nonlocal nrows, total_nnz, last_rank_update_seconds
        added_rows = 0
        added_nnz = 0
        rank_t0 = time.time()
        stage_hash = hashlib.sha256() if rc3_run_dir is not None else None
        row_hash_index_file = f"{safe_stage_name(stage)}.rowhashes.jsonl" if rc3_run_dir is not None else None
        row_hash_handle = None
        row_hash_path = (rc3_run_dir / row_hash_index_file) if rc3_run_dir is not None and row_hash_index_file else None
        if row_hash_path is not None:
            row_hash_handle = row_hash_path.open("w", encoding="utf-8")
        try:
            for item in row_iter:
                row, row_metadata = unpack_row_item(item)
                if not row:
                    continue
                row_line_sha256 = None
                if stage_hash is not None:
                    line = f"{stage}\t{added_rows}\t{canonical_row(row)}\n"
                    line_bytes = line.encode("utf-8")
                    row_line_sha256 = hashlib.sha256(line_bytes).hexdigest()
                    stage_hash.update(line_bytes)
                    if row_hash_handle is not None:
                        row_hash_handle.write(json.dumps(
                            {
                                "stage_row_index": added_rows,
                                "row_line_sha256": row_line_sha256,
                            },
                            sort_keys=True,
                        ) + "\n")
                stage_row_index = added_rows
                if quotient_ranker is not None and quotient_col_map is not None and py_ranker is not None:
                    ...
                    if projected:
                        if quotient_ranker.add(projected):
                            write_rc3_source_row(stage, stage_row_index, row, row_line_sha256, row_metadata)
    ...
    def manin_relation_rows() -> Iterator[tuple[dict[str, Any], dict[int, Any]]]:
        for i in range(nsyms):
            terms: list[tuple[int, Any]] = [(i, F(1))]
            terms.extend(syms.apply_T(i))
            terms.extend(syms.apply_TT(i))
            yield {"source_kind": "manin_T", "manin_symbol_index": i}, reduce_terms(terms)
    ...
    def hecke_relation_rows(p: int, ap: int) -> Iterator[tuple[dict[str, Any], dict[int, Any]]]:
        ...
        for i in hecke_indices(p):
            ...
            metadata = {
                ...
                "hecke_family": hecke_family,
                "hecke_row_order": hecke_row_order,
                "manin_symbol_index": int(i),
            }
            yield metadata, reduce_terms(terms)
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    parser.add_argument("--raw-a", type=int, default=RAW_A, help="Frey raw orientation a in y^2=x(x-a)(x+b).")
    parser.add_argument("--raw-b", type=int, default=RAW_B, help="Frey raw orientation b in y^2=x(x-a)(x+b).")
...
def main(argv: list[str] | None = None) -> int:
    ...
    global RAW_A, RAW_B
    RAW_A = int(args.raw_a)
    RAW_B = int(args.raw_b)
    ...
    payload = {
        ...
        "rank_algorithm": args.rank_algorithm,
        "hecke_batch_size": args.hecke_batch_size,
        "max_hecke_batches_per_prime": args.max_hecke_batches_per_prime,
        "pivot_strategy": args.pivot_strategy,
        "hecke_row_order": args.hecke_row_order,
        "hecke_row_seed": args.hecke_row_seed,
        "skip_existing_pivot": args.skip_existing_pivot,
        "runs": runs,
    }
